...
import { 
  getWeatherEmoji, getWeatherDescription, formatTemp, formatTime, 
  formatDate, formatDateFull, getUVLevel, getWindLevel,
  getTempColor, capitalize, escapeHtml, getLocalTodayString
} from './utils.js';
...
export function renderDaySelector(dailyData, selectedDateStr, onSelect, unit = 'celsius') {
  ...
  elements.daySelector.innerHTML = dailyData.map(day => {
    ...
    const precipHtml = day.precipProbMax > 0
      ? `<span class="day-precip" style="font-size: 0.75rem; color: #38bdf8; margin-top: 2px;">💧${day.precipProbMax}%</span>`
      : `<span class="day-precip" style="font-size: 0.75rem; color: #38bdf8; margin-top: 2px; opacity: 0.4;">💧0%</span>`;
...
export function renderTimeline(hourlyData, selectedDateStr, unit = 'celsius', startHour = -1, endHour = -1) {
  ...
  elements.timeline.innerHTML = hourlyData.map(h => {
    ...
    const precipVal = h.precipProb !== undefined ? h.precipProb : 0;
    const precip = `<span class="hour-precip" style="opacity: ${precipVal > 0 ? 1 : 0.4}">💧${precipVal}%</span>`;
